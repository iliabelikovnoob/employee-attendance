...
export default function RecentPage() {
  ...
  const fetchRecent = async () => {
    try {
      const response = await fetch('/api/kb/recent?limit=20');
      if (response.ok) {
    ...
    return (
      <div className="p-8 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-300">Загрузка недавних статей...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      <Breadcrumbs items={[{ label: 'Недавние', icon: '🕒' }]} />
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2 flex items-center gap-3">
          <IoTimeOutline className="w-8 h-8 text-blue-500" />
          Недавние статьи
        </h1>
        <p className="text-gray-600 dark:text-gray-400">
          Последние добавленные статьи в базу знаний
        </p>
      </div>
      ...
      {articles.length === 0 ? (
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-12 border border-gray-200 dark:border-gray-700 text-center">
          <IoTimeOutline className="w-16 h-16 text-gray-400 dark:text-gray-500 mx-auto mb-4" />
