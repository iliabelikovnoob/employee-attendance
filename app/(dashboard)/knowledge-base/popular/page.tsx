...
export default function PopularPage() {
  ...
  const fetchPopular = async () => {
    try {
      const response = await fetch('/api/kb/popular?limit=20');
      if (response.ok) {
    ...
    return (
      <div className="p-8 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-300">Загрузка популярных статей...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      <Breadcrumbs items={[{ label: 'Популярное', icon: '🔥' }]} />
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2 flex items-center gap-3">
          <IoFlameOutline className="w-8 h-8 text-orange-500" />
          Популярные статьи
        </h1>
        <p className="text-gray-600 dark:text-gray-400">
          Самые просматриваемые статьи за последний месяц
        </p>
      </div>
      ...
      {articles.length === 0 ? (
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-12 border border-gray-200 dark:border-gray-700 text-center">
          <IoFlameOutline className="w-16 h-16 text-gray-400 dark:text-gray-500 mx-auto mb-4" />
          <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">
            Пока нет популярных статей
          </h3>
          <p className="text-gray-500 dark:text-gray-400 mb-6">
            Создайте статьи и они появятся здесь
          </p>
          <Link
        ...
        <div className="grid gap-4">
          {articles.map((article, index) => (
            <Link
              ...
              className="bg-white dark:bg-gray-800 rounded-xl shadow-md hover:shadow-lg border border-gray-200 dark:border-gray-700 p-6 transition-all hover:scale-[1.01] relative"
            >
              {/* Рейтинг */}
